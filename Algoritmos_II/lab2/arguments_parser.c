/*!
  @file arguments_parser.c
  @brief arguments_parser methods definition
*/
...
static int ketopt(ketopt_t * s, int argc, char *argv[], int permute,
                  const char *ostr, const ko_longopt_t * longopts)
{
    int opt = -1, i0, j;
    if (permute) {
        while (s->i < argc
               && (argv[s->i][0] != '-' || argv[s->i][1] == '\0'))
            ++s->i, ++s->n_args;
    }
    ...
    if (argv[s->i][0] == '-' && argv[s->i][1] == '-') { /* "--" or a long option */
        ...
        if (longopts) {         /* parse long options */
            ...
            for (j = 2; argv[s->i][j] != '\0' && argv[s->i][j] != '='; ++j) {
            }                   /* find the end of the option name */
            for (k = 0; longopts[k].name != 0; ++k)
                if (strncmp(&argv[s->i][2], longopts[k].name, j - 2) == 0) {
                    if (longopts[k].name[j - 2] == 0)
                        ++n_exact, o_exact = &longopts[k];
                    else
                        ++n_partial, o_partial = &longopts[k];
                }
...
{
    parse_arguments_result result;
    result.do_selection_sort = false;
    result.do_insertion_sort = false;
    result.do_quick_sort = false;
    result.do_merge_sort = false;
    result.do_not_sort = true;
    ...
    int c;
    while ((c = ketopt(&opt, argc, argv, 1, "isqmcp", longopts)) >= 0) {
        switch (c) {
        case 'i':{
                result.do_insertion_sort = true;
                result.do_not_sort = false;
                break;
            }
        case 's':{
                result.do_selection_sort = true;
                result.do_not_sort = false;
                break;
            }
        case 'q':{
                result.do_quick_sort = true;
                result.do_not_sort = false;
                break;
            }
	    case 'm':{
		        result.do_merge_sort = true;
		        result.do_not_sort = false;
		        break;
	        }
        case 'c':
